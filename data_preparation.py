...
all_parquets = glob.glob("total_df_*.parquet.gzip")

print(all_parquets)

#%%
# find the latest df among all data sets
...
InputOutputArable = pd.read_excel('InputOutput.xlsx', engine='openpyxl', index_col=0)  
...
All_names = [x for x in All_names if x == x]
...
#%%
# Save the min max and number of empty cells in an excel
X_min = X_all.min(axis=0)
X_max = X_all.max(axis=0)
X_empty_count = X_all.isnull().sum()
...
X_train_raw, X_test_raw, Y_train_raw, Y_test_raw = train_test_split(
    X_all, Y_all, test_size=0.1, random_state=101)
...
for df, j in zip(my_list, range(8)):
    ...
    df.to_parquet(filename+'.parquet.gzip', engine='pyarrow', compression='gzip', index=False)
